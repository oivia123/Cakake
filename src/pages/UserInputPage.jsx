...
const UserInputPage = () => {
  ...
  useEffect(() => {
    if (pendingSummary) {
      ...
      // 过敏信息直接用pendingSummary.allergies
      ...
      // 缺货信息和蛋糕选项全局占位（如window.inventory），如无则传默认蛋糕列表
      const inventory = window.inventory || {
        missingIngredients: [],
        availableCakes: defaultCakes
      };
      if (!inventory.availableCakes || !inventory.availableCakes.length) {
        inventory.availableCakes = defaultCakes;
      }
      generateCakeDescription({ tarotCards, allergies, inventory })
        .then(description => {
          // 跳转到结果页并传递结果、塔罗牌、用户输入
          navigate('/result', { state: { result: { description, tarotCards, note: pendingSummary.note } } });
        });
    }
  }, [pendingSummary]);
